...
class DataLoader:
    def __init__(
        self, data_folder: str, make_folder: bool, save_folder_name: str = None, server_mount_drive: str = "S:"
    ):
    ...
    def get_animal_id(self):
        ...
        if match:
            ...
            raise ValueError(f"ICMS* pattern not found in the path: {self.data_folder}")
    ...
    def get_save_folder(self):
        if self.save_folder_name:
            ...
            if self.make_folder:
                ...
                print(f"Save folder created or confirmed at: {self.save_folder}")
            elif not self.save_folder.exists():
                # If folder creation is not requested but the folder does not exist, raise an error
                raise ValueError(f"Specified save folder does not exist and make_folder is False: {self.save_folder}")
            ...
            if self.make_folder:
                ...
                self.save_folder = self.data_folder / f"Processed_{current_date_string}"
                os.makedirs(self.save_folder, exist_ok=True)
    ...
    def find_newest_processed_folder(self):
        processed_folders = [f for f in os.listdir(self.data_folder) if f.startswith("Processed_")]
        if processed_folders:
            ...
            newest_folder = max(processed_folders, key=lambda x: parse_folder_date(x))
    ...
    def remove_high_impedance_ch(self):
        ...
        impedance_analyzer.get_intan_impedances(animal_id=self.animalID, server_mount_drive=self.server_mount_drive)
        filtered_impedances, good_channels = impedance_analyzer.get_good_impedances(threshold=2e6)
        ...
        self.recording = self.recording.channel_slice(channel_ids=channel_ids[good_indices])

    def attach_probe(self):
        pi = read_probeinterface("util\\net32Ch.json")
        probe = pi.probes[0]
    ...
    def get_base_server_folder(self):
        ...
        self.base_server_folder = os.path.join(self.server_mount_drive, icms_id, "Keys", date, subfolder)
    ...
    def get_ns5_and_mat_files(self, first_N_files=4):
        ...
        self.mat_files = [f for f in all_files if re.match(r"block\d+_H\d+M\d+_behavior.mat", f)]
        self.mat_files = self.mat_files[:n_files]
        base_names = [re.match(r"(block\d+_H\d+M\d+)_behavior.mat", f).group(1) for f in self.mat_files]
        self.ns5_files = [self._get_ns5_file_matching_base(base_name, all_files) for base_name in base_names]

    def get_key_files(self):
        """Extract key information from server folder."""
        key_paths = [os.path.join(self.base_server_folder, f.replace("_behavior", "_key")) for f in self.mat_files]
        (
            self.all_currents,
            self.all_depths,
        ...
        flattened_list = [item for sublist in self.all_stim_timestamps for item in sublist]
        assert flattened_list == sorted(flattened_list)
        flattened_list = [item for sublist in self.all_stim_start_stop for item in sublist]
        assert flattened_list == sorted(flattened_list)
        flattened_list = [item for sublist in self.all_image_start_stop for item in sublist]
        assert flattened_list == sorted(flattened_list)
    ...
    def check_premature_recording_stop(self, stim_start_stop):
        ...
        for index, sublist in enumerate(stim_start_stop):
            ...
            return -1
        last_trial = index - 1
        print(f"Prematurely stopped ephys recording found: using trials 1-{last_trial+1}")
        return last_trial
    ...
    def _extract_key_data_from_mat(self, mat_data, index):
        ...
        stim_timestamps = [
            (np.array(trial["StimTimestamps"]) * self.fs + self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]
        stim_start_stop = [
            (np.array(trial["StimStartStop"]) * self.fs + self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]
        img_start_stop = [
            (np.array(trial["ImageStartStop"]) * self.fs + self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]
        frame_timestamps = [
            (np.array(trial["FrameTimestamps"]) * self.fs + self.start_times[index] * self.fs).tolist()
            for trial in trial_array
        ]
        ...
        if last_trial > -1:
            ...
            self.modified_stim_timestamps = self.fill_with_nans(last_trial, stim_timestamps)
            self.modified_frame_timestamps = self.fill_with_nans(last_trial, frame_timestamps)
            self.modified_img_start_stop = self.fill_with_nans(last_trial, img_start_stop)

            self.all_modified_stim_timestamps.extend(self.modified_stim_timestamps)
            self.all_modified_frame_timestamps.extend(self.modified_frame_timestamps)
            self.all_modified_img_start_stop.extend(self.modified_img_start_stop)
        ...
        if type(img_start_stop[-1]) != list:
            img_start_stop[-1] = [img_start_stop[-1], img_start_stop[-1] + 30000 * 5]
    ...
    def get_fs(self):
        ...
        rec_1 = se.read_blackrock(file_path=file_path, stream_id="5")
    ...
    def get_start_times(self):
        ...
        for i, ns5_file in enumerate(self.ns5_files):
            ...
            rec_i = se.read_blackrock(file_path=file_path, stream_id="5")
    ...
    def append_recordings(self):
        ...
        durations = [rec_i.get_num_frames() / self.fs for rec_i in self.recordings_list]
        self.start_times = [0] + np.cumsum(durations[:-1]).tolist()

        self.time_vectors = [
            self.get_time_vector(rec_i, start_time) for rec_i, start_time in zip(self.recordings_list, self.start_times)
        ]
    ...
    def _correct_depths(self):
        ...
        if any(depth > 20 for depth in self.all_depths):
            ...
            ripple_ids = [ripple_id for ripple_id in ripple_ids if ripple_id != 0]
            depth_idx = ia.ripple_to_depth2(ripple_ids)
            mapping = dict(zip(ripple_ids + [0], depth_idx + [0]))
            self.all_depths = [mapping.get(val, val) for val in self.all_depths]
        ...
        trial_df = pd.DataFrame(
            {
                ...
                "image_start": [
                    sublist[0] if isinstance(sublist, list) and sublist != [np.nan] else np.nan
                    for sublist in df_img_start_stop
                ],
                "image_stop": [
                    sublist[1] if isinstance(sublist, list) and sublist != [np.nan] else np.nan
                    for sublist in df_img_start_stop
                ],
            }
        )
    ...
    def get_block_detection_threshold(self):
        ...
        for block in range(num_blocks):
            ...
            unique_channels = sub_df[sub_df["channel"] != 0]["channel"].unique()
            ...
            for channel in unique_channels:
                ...
                grouped_data = (
                    channel_data.groupby("current")
                    .agg(responses=("response", "sum"), total_trials=("trial", "count"))
                    .reset_index()
                )

                psignifit_data = np.column_stack(
                    [grouped_data["current"], grouped_data["responses"], grouped_data["total_trials"]]
                )
                psignifit_data = np.row_stack([catch_trial_row, psignifit_data])
