...
from util.load_data import DataLoader, get_dataframe
...
def process_thresholds(data):
    ...
    # Sort the dates
    sorted_dates = sorted(data, key=lambda x: datetime.datetime.strptime(x.split("\\")[-1], "%d-%b-%Y"))

    # Earliest date for relative day calculation
    earliest_date = datetime.datetime.strptime(sorted_dates[0].split("\\")[-1], "%d-%b-%Y")
    ...
    df = pd.DataFrame(
        {
            "Date": dates,
            "Date" "Relative Day": relative_days,
            "Block": blocks,
            "Channel": channels,
...
def plot_thresholds_over_time(df):
    ...
    sns.lineplot(data=df, x="Date", y="Threshold", hue="Channel", errorbar="sd", palette=palette)
...
def get_hits_and_currents_per_ch(df):
    ...
    for block in range(4):
        ...
        unique_channels = sorted(sub_df[sub_df["channel"] != 0]["channel"].unique())
        channel_hits = {}
        channel_currents = {}

        for channel in unique_channels:
            channel_data = sub_df[sub_df["channel"] == channel]
            channel_currents[channel] = np.sort(channel_data["current"].unique()).tolist()
            current_hits = []
        ...
        hits_and_currents_per_ch[block] = {"hits": channel_hits, "currents": channel_currents}
...
def is_non_decreasing_with_exceptions(
    hits, allowed_exceptions=1, min_increasing_or_constant_steps=1, max_allowed_drop=3
):
    exceptions = sum(y < x for x, y in zip(hits, hits[1:]))
    increasing_or_constant_steps = sum(y >= x for x, y in zip(hits, hits[1:]))
    largest_drop = max((x - y) for x, y in zip(hits, hits[1:])) if exceptions > 0 else 0
    return (
        exceptions <= allowed_exceptions
        and increasing_or_constant_steps >= min_increasing_or_constant_steps
        and largest_drop <= max_allowed_drop
    )


def inclusion_criteria(ch_a_hits, ch_b_hits, ch_c_hits):
    channels_with_sufficient_hits = sum(max(hits) >= 4 for hits in [ch_a_hits, ch_b_hits, ch_c_hits])
    channels_with_acceptable_trend = sum(
        is_non_decreasing_with_exceptions(hits) for hits in [ch_a_hits, ch_b_hits, ch_c_hits]
    )
    ...
    with open(csv_file_name, "w", newline="") as csvfile:
        ...
        writer.writerow(
            [
                ...
                "HitsA",
                "ThresholdA",
                "Closest CurrentA to 50%",
                "ChB",
                "CurrentsB",
                "HitsB",
                "ThresholdB",
                "Closest CurrentB to 50%",
                "ChC",
                "CurrentsC",
                "HitsC",
                "ThresholdC",
                "Closest CurrentC to 50%",
                "Include",
            ]
        )
        ...
        for session, blocks in all_session_thresholds.items():
            ...
            sorted_channels = sorted(block_thresholds.items(), key=lambda x: x[0])
            channel_numbers = [channel[0] for channel in sorted_channels]
            thresholds = [channel[1] for channel in sorted_channels]

            ch_a_hits = list(block_hits_and_currents["hits"].get(channel_numbers[0], 0))
            ch_b_hits = list(block_hits_and_currents["hits"].get(channel_numbers[1], 0))
            ch_c_hits = list(block_hits_and_currents["hits"].get(channel_numbers[2], 0))

            ch_a_curr = block_hits_and_currents["currents"].get(channel_numbers[0], [])
            ch_b_curr = block_hits_and_currents["currents"].get(channel_numbers[1], [])
            ch_c_curr = block_hits_and_currents["currents"].get(channel_numbers[2], [])
            ...
            if include == 1:
                if session not in session_data:
                    session_data[session] = {
                        ...
                        "threshold_sums": [0.0, 0.0, 0.0],
                        "current_counts": [0, 0, 0],
                        "current_sums": [0.0, 0.0, 0.0],
                    }
                for i, ch in enumerate("ABC"):
                    session_data[session]["threshold_counts"][i] += 1
                    session_data[session]["threshold_sums"][i] += float(row[f"Threshold{ch}"])
                    session_data[session]["current_counts"][i] += 1
                    session_data[session]["current_sums"][i] += float(row[f"Closest Current{ch} to 50%"])
    ...
    with open(csv_file_name, "r", newline="") as csvfile:
        ...
        for row in reader:
            ...
            if session in session_data and block == 1:
                avg_thresholds = [
                    int(round(sums / counts, 0)) if counts > 0 else 0
                    for counts, sums in zip(
                ...
                delta = [round(threshold - current, 2) for threshold, current in zip(avg_thresholds, avg_currents)]
                row["Avg Thresholds"] = "[" + ", ".join([str(avg) for avg in avg_thresholds]) + "]"
                row["Avg Closest Currents"] = "[" + ", ".join([str(avg) for avg in avg_currents]) + "]"
...
def get_currents_per_ch(df):
    ...
    for block in range(4):
        ...
        for channel in unique_channels:
            channel_data = sub_df[sub_df["channel"] == channel]
            channel_currents[channel] = channel_data["current"].unique().tolist()
...
def get_total_hits_dict(sorted_data_folders):
    ...
    for data_folder in sorted_data_folders:
        df = get_dataframe(data_folder)
        total_hits_per_ch, total_max_cur_hits_per_ch = get_total_hits_per_ch(df)
    ...
    for session_idx, (date, block_dict) in enumerate(hits_dict.items()):
        for current_idx in range(3):
            ...
            for block_idx, (block, channels) in enumerate(block_dict.items()):
                ...
                current_values = [channel_data[current_idx] for channel_data in channels.values()]
...
def plot_included_block_thresholds_over_time(csv_file_name):
    ...
    df["Date"] = df["DateString"].apply(lambda x: datetime.strptime(x, "%d-%b-%Y"))
    ...
    date_mapping = {date.strftime("%d-%b-%Y"): i for i, date in enumerate(sorted_dates)}
    ...
    closest_current_columns = ["Closest CurrentA to 50%", "Closest CurrentB to 50%", "Closest CurrentC to 50%"]
    jitter_amount = 0.1
    for i, (threshold_column, closest_current_column) in enumerate(zip(threshold_columns, closest_current_columns)):
        color = palette[i]
        jittered_x = [
            date_mapping[date] + np.random.uniform(-jitter_amount, jitter_amount) for date in df_filtered["DateString"]
        ]
        ax.scatter(jittered_x, df_filtered[threshold_column], color=color)
        df_filtered["DateIndex"] = df_filtered["DateString"].map(date_mapping)
        average_thresholds = df_filtered.groupby("DateIndex")[threshold_column].mean()
        average_thresholds.plot(kind="line", color=color, ax=ax, marker="o", linestyle="-")
        # Plot the closest current to 50% as dotted lines
        ax.plot(
            df_filtered["DateIndex"],
            df_filtered[closest_current_column],
            color=color,
            linestyle="--",
            linewidth=2,
            label=f"{closest_current_column}",
        )
...
if __name__ == "__main__":
    ...
    all_animals_dict[animalID] = {"sorted_data_folders": sorted_data_folders, "hc_dict": hits_and_currents_dict}
