...
class PPTImageInserter:
    ...
    def __init__(self, grid_dims=(3, 3), spacing=(0.05, 0.05), title_font_size=16):
        ...
        self.available_width = (
            self.slide_width - (self.grid_cols + 1) * self.spacing_horiz
        )
        self.available_height = (
            self.slide_height
            - self.title_height
            - 0.2
            - (self.grid_rows + 1) * self.spacing_vert
        )
    ...
    def add_slide(self, slide_title=None):
        if slide_title is None:
            ...
            title_shape.text_frame.paragraphs[0].font.size = Pt(self.title_font_size)
    ...
    def add_image(self, image_path, slide_title=None):
        ...
        left = Inches(
            col * (self.image_width + self.spacing_horiz) + self.spacing_horiz
        )
        top = Inches(
            self.title_height
            + 0.2
            + row * (self.image_height + self.spacing_vert)
            + self.spacing_vert
        )
        ...
        if img_aspect > 1:
            # Wide image
            width = Inches(min(self.image_width, self.image_height * img_aspect))
            height = width / img_aspect
        else:
            # Tall image
            height = Inches(min(self.image_height, self.image_width / img_aspect))
            width = height * img_aspect

        self.slide.shapes.add_picture(image_path, left, top, width=width, height=height)
        self.image_index += 1
